...
import css from './product-sales.css' assert { type: 'css' };

export class ProductSales extends Dashboard {
  ...
  @property() chartType: 'bar' | 'line' | 'pie' = 'bar';
  ...
  #renderChart(sales: ProductSale[]): TemplateResult {
    ...
    return html`
      <rk-chart-element
        .data=${data}
        .type=${this.chartType}></rk-chart-element>
    `;
  }
...
if (!customElements.get('app-product-sales')) {
  customElements.define('app-product-sales', ProductSales);
}

declare global {
  interface HTMLElementTagNameMap {
    'app-product-sales': ProductSales;
  }
}
