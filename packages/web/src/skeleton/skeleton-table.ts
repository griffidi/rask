...
export class SkeletonTable extends LitElement {
  ...
  override render(): TemplateResult {
    return html`
      <rk-table>${this.#renderHeader()} ${this.#renderRows()}</rk-table>
    `;
  }
  ...
  #renderRows(): TemplateResult {
    return html`
      ${map(
        ...
        () =>
          html`
            <rk-table-row>${this.#renderTableCells()}</rk-table-row>
          `
      )}
    `;
  }
...
if (!customElements.get('rk-skeleton-table')) {
  customElements.define('rk-skeleton-table', SkeletonTable);
}
